...
const router = createRouter({
  ...
  routes: [
    ...
    },
    {
      path: '/add-interface',
      name: 'Add Interface',
      component: () => import('@/containers/AddInterface.vue'),
      beforeEnter: (to, from) => {
        const checkRoles = () => {
          if (!adminRole.value) {
            showSnackBar({ msg: 'Must be admin to add interfaces.' })
            router.push(from.path)
          }
        }
        if (rolesAreLoaded.value) checkRoles()
    ...
    },
    {
      ...
      beforeEnter: (to, from) => {
        const checkRoles = () => {
          if (!adminRole.value) {
            showSnackBar({ msg: 'Must be admin to access Path Outages.' })
            router.push(from.path)
          }
        }
        if (rolesAreLoaded.value) checkRoles()
    ...
    },
    {
      ...
      beforeEnter: (to, from) => {
        const checkRoles = () => {
          if (!adminRole.value) {
            showSnackBar({ msg: 'Must be admin to access On-Call Roles.' })
            router.push(from.path)
          }
        }
        if (rolesAreLoaded.value) checkRoles()
    ...
    },
    {
      ...
      name: 'On-Call Role Detail',
      component: () => import('@/containers/OnCallRoleDetail.vue'),
      beforeEnter: (to, from) => {
        const checkRoles = () => {
          if (!adminRole.value) {
            showSnackBar({ msg: 'Must be admin to access On-Call Role Detail.' })
            router.push(from.path)
          }
        }
        if (rolesAreLoaded.value) checkRoles()
