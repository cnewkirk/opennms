  ...
  <div class="topology-toolbar">
    ...
    <div class="topology-toolbar__views" ref="viewsPanelRef">
      ...
      <div v-if="viewsPanelOpen" class="topology-toolbar__views-panel">
        ...
        <!-- Per-user server-side views + browser-local private views -->
    ...
    <div class="topology-toolbar__edge-labels" ref="edgeLabelPanelRef">
      ...
      <div v-if="edgeLabelPanelOpen" class="topology-toolbar__edge-label-panel">
        <label class="topology-toolbar__edge-label-row">
          <input type="checkbox" v-model="viewStore.edgeLabels.showUtilization"> Utilization
        </label>
        <label class="topology-toolbar__edge-label-row">
          <input type="checkbox" v-model="viewStore.edgeLabels.showLocalPort"> Local Port
        </label>
        <label class="topology-toolbar__edge-label-row">
          <input type="checkbox" v-model="viewStore.edgeLabels.showRemotePort"> Remote Port
        </label>
        <label class="topology-toolbar__edge-label-row">
          <input type="checkbox" v-model="viewStore.edgeLabels.showIp"> IP Addresses
        </label>
        <label class="topology-toolbar__edge-label-row">
          <input type="checkbox" v-model="viewStore.edgeLabels.showMac"> MAC Address
        </label>
        <label class="topology-toolbar__edge-label-row">
          <input type="checkbox" v-model="viewStore.edgeLabels.showSpeed"> Speed
        </label>
      </div>
    </div>
  </div>
...
/** Called by the parent container after a server-side view is saved/deleted to refresh lists. */
...
const toggleCategory = async (cat: string) => {
  ...
  // Discard result if a newer call has already completed
...
const clearAllFilters = async () => {
  ...
  await store.fetchCategoryNodeIds([])
  viewStore.markDirty()
}
...
// Load server views and categories on mount
onMounted(async () => {
...
.topology-toolbar {
  ...
  &__views-action {
    ...
    &:hover { background: var($shade-2, rgba(0,0,0,0.06)); }
  }
  ...
  &__cidr-chip {
    ...
    border-radius: vars.$border-radius-pill;
    font-size: 0.75rem;
    background: var($shade-2, rgba(0,0,0,0.08));
    color: var($primary-text-on-surface);
